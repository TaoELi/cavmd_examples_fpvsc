...
def plot_IR_all():
    ...
    '''
    fig, axes = clp.initialize(1, 2, width=4.3*2, height=4.3*0.7,  LaTeX=True, fontsize=12, return_fig_args=True, labelthem=True, labelsize=13, labelthemPosition=[-0.05, 1.06])
    ...
    E0 = "5e-5"
    paths = ["../co2_liquid_phase/E0_%s" %E0, "../co2_liquid_phase/cavlifetime_300"]
    for i, path in enumerate(paths):
        freq, sp = obtain_spectrum(path=path, subspectrum=True, n=1)
        xs.append(freq)
        ys.append(sp / np.max(sp) +  (i+1)*1.)
    colors = ["k"] * len(path)
    clp.plotone(xs, ys, axes[0], colors=colors, showlegend=False, lw=1, xlim=[1900, 2650], ylim=[-0.01, 3.05],
    ...
    axes[0].text(1910, 0.1, "liquid CO$_2$\noutside cavity", fontsize=9.5, color=colors[0])
    axes[0].text(1910, 1.1, "36-mode lossless cavity\n36 CO$_2$ subsystems",fontsize=9.5, color=colors[1])
    axes[0].text(1910, 2.1, "36-mode lossy cavity\n36 CO$_2$ subsystems",fontsize=9.5, color=colors[2])
    axes[0].text(1910, 2.7, "Classical",fontsize=12)
    # plot the cavity structure
    axes[0].axvline(x=2320, lw=3, color=clp.dark_green, alpha=0.5)

    # Figure b: Cavity detuning dependence of the IR spectrum
    N = 36
    E0 = "5e-5"
    freq, sp = obtain_spectrum(path="../co2_liquid_phase/cavlifetime_300/", n=N)
    ...
    axes[1].text(1200, 2484, "cavity photon", color='g', fontsize=12)
    axes[1].text(1000, 2355, "C=O asym. stretch", color='w', fontsize=12)
    ...
    clp.adjust(savefile="IR_CO2_cavloss.pdf")
