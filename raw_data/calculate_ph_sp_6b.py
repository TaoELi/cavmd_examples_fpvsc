...
def get_dipole_spectrum(xyz_filename, save2file=False, n_co2=36*6, n_ph=36, dt_fs=2):
    # load xyz file
    ...
    if save2file:
        ...
        data = np.zeros((dacf_y_freq.size, n_ph+1))
        data[:,0] = dacf_y_freq
        for j in range(n_ph):
            data[:,1+j] = sps[j]
...
for xyz_filename in xyz_filenames:
    ...
    try:
        ...
        path = Path(xyz_filename+".phsp")
        if path.is_file():
            print(xyz_filename, "done!")
            continue
        freq, sp = get_dipole_spectrum(xyz_filename=xyz_filename, save2file=True, n_ph=n_ph)
        xs.append(freq)
        ys.append(sp)
    except:
        print("Error!!!")
